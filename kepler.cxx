...
int main(){
  ...
  double p_2 = sqrt( (1.0+e)/(1.0-e) );
	...
	out << H << "\t" << 0 << "\t" << p_1 << "\t" << p_2 << "\t" << q_1 << "\t" << q_2 << endl;
  for(double t=0; t<=tEnd; t+=dt)
  {
  	tmp_p_1 = p_1 - dt*( q_1/( pow( pow(q_1, 2) + pow(q_2, 2),3.0/2.0) ) );
  	tmp_p_2 = p_2 - dt*( q_2/( pow( pow(q_1, 2) + pow(q_2, 2),3.0/2.0) ) );
  	...
  	out << H << "\t" << t  << "\t" << tmp_p_1 << "\t" << tmp_p_2 << "\t" << tmp_q_1 << "\t" << tmp_q_2 <<  endl;
